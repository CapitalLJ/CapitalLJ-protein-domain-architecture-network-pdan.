...
def map_species_to_da(species_proteins, da_compositions):
    ...
    with open("tmp.txt",'w') as file:
        ...
        for i in species_to_da:
            file.write("{}\t{}\n".format(i,species_to_da[i]))
    return species_to_da


def check_pidan_da_presence(species_to_da, pidan_compositions):
    results = []
    all_da = set(da for das in species_to_da.values() for da in das)
    for pidan, das in pidan_compositions.items():
        presence = 1 if any(da in all_da for da in das) else 0
        results.append((pidan, presence))
    return results
...
def write_results(input_file,file_path, results):
    ...
    with open(file_path, 'w') as file:
        ...
        for pidan, presence in results:
            file.write(f"{pidan}\t{presence}\n")
...
if __name__ == "__main__":
    ...
    # 检查每一个pidan的DA是否出现过
    results = check_pidan_da_presence(species_to_da, pidan_compositions)
